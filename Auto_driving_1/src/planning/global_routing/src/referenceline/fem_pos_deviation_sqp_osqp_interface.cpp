...
bool FemPosDeviationSqpOsqpInterface::Solve()
{
  ...
  while (pen_itr < sqp_pen_max_iter_)
  {
    ...
    while (sub_itr < sqp_sub_max_iter_)
    {
      ...
      {
        std::cout << "iteration at " << sub_itr << ", solving fails with max sub iter " << sqp_sub_max_iter_;
      ...
      if (ftol < sqp_ftol_)
      {
        ...
        std::cout << "merit function value converges to " << cur_fvalue << ", with ftol " << ftol << ", under max_ftol "
                  << sqp_ftol_;
        fconverged = true;
        break;
      }
    ...
    if (ctol < sqp_ctol_)
    {
      ...
      std::cout << "constraint voilation value drops to " << ctol << ", under max_ctol " << sqp_ctol_;
      weight_curvature_constraint_slack_var_ = original_slack_penalty;
      osqp_cleanup(work);
      c_free(data->A);
      c_free(data->P);
  ...
  std::cout << "constraint voilation value drops to " << ctol << ", higher than max_ctol " << sqp_ctol_;
  weight_curvature_constraint_slack_var_ = original_slack_penalty;
  osqp_cleanup(work);
  c_free(data->A);
  c_free(data->P);
                                                      ...
                                                      std::vector<c_int>* P_indptr)
{
  ...
  for (int point_index = 2; point_index < second_point_from_last_index; ++point_index)
  {
    ...
    for (int col = 0; col < 2; ++col)
    {
      ...
      columns[col_index].emplace_back(col_index, 6.0 * weight_fem_pos_deviation_ + 2.0 * weight_path_length_ +
...
double
FemPosDeviationSqpOsqpInterface::CalculateConstraintViolation(const std::vector<std::pair<double, double>>& points)
{
