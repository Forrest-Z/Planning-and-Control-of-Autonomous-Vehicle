...
ActiveSetQpSolver::ActiveSetQpSolver(const Eigen::MatrixXd &kernel_matrix, const Eigen::MatrixXd &offset,
    ...
    : QpSolver(kernel_matrix, offset, affine_inequality_matrix, affine_inequality_boundary, affine_equality_matrix,
      ...
      num_constraint_(static_cast<int>(affine_equality_matrix_.rows() + affine_inequality_matrix_.rows())), num_param_(static_cast<int>(kernel_matrix.rows())), qp_eps_num_(Config_.default_active_set_eps_num), qp_eps_den_(Config_.default_active_set_eps_den), qp_eps_iter_ref_(Config_.default_active_set_eps_iter_ref), debug_info_(Config_.default_enable_active_set_debug_info)
{
}
...
bool ActiveSetQpSolver::Solve()
{
  ...
  {
    // std::cout << "kernel_matrix_.rows() [" << kernel_matrix_.rows() << "] and kernel_matrix_.cols() ["
    //    << kernel_matrix_.cols() << "] should be identical.";
    return false;
  }
  // definition of qpOASESproblem
  const auto kNumOfMatrixElements = kernel_matrix_.rows() * kernel_matrix_.cols();
  ...
  {
    constraint_lower_bound[r + affine_equality_boundary_.rows()] = affine_inequality_boundary_(r, 0);
  ...
  if (ret != qpOASES::SUCCESSFUL_RETURN)
  {
    ...
    // std::cout << "ActiveSetQpSolver inputs: " << std::endl;
    // std::cout << "kernel_matrix:\n" << kernel_matrix_ << std::endl;
    // std::cout << "offset:\n" << offset_ << std::endl;
    // std::cout << "affine_inequality_matrix:\n" << affine_inequality_matrix_ << std::endl;
    // std::cout << "affine_inequality_boundary:\n" << affine_inequality_boundary_ << std::endl;
    // std::cout << "affine_equality_matrix:\n" << affine_equality_matrix_ << std::endl;
    // std::cout << "affine_equality_boundary:\n" << affine_equality_boundary_ << std::endl;
...
{
  return kernel_matrix_.rows() == kernel_matrix_.cols() && kernel_matrix_.rows() == affine_inequality_matrix_.cols() &&
