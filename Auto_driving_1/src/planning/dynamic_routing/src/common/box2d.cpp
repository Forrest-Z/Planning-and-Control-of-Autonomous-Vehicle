...
double Box2d::DistanceTo(const LineSegment2d &line_segment) const
{
    ...
    case 2:
      return (x1 > x2) ? PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                       ...
                       : PtSegDistance(box_x, box_y, x1, y1, x2, y2,
                                       line_segment.length());
    case -1:
      return common::math::CrossProd({x1, y1}, {x2, y2}, {box_x, -box_y}) >= 0.0
                 ? 0.0
                 : PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                                 line_segment.length());
    case -4:
      return common::math::CrossProd({x1, y1}, {x2, y2}, {box_x, -box_y}) <= 0.0
                 ? PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                                 line_segment.length())
                 : (common::math::CrossProd({x1, y1}, {x2, y2}, {-box_x, box_y}) <= 0.0
                        ? 0.0
                        : PtSegDistance(-box_x, box_y, x1, y1, x2, y2,
                                        line_segment.length()));
    }
  ...
  // std::cout << "unimplemented state: " << gx1 << " " << gy1 << " " << gx2 << " " << gy2;
  return 0.0;
}

double Box2d::DistanceTo(const Box2d &box) const
{
  return common::math::Polygon2d(box).DistanceTo(*this);
}
