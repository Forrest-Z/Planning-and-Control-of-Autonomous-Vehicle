...
bool Polygon2d::IsPointIn(const Vec2d& point) const
{
  ...
  for (int i = 0; i < num_points_; ++i)
  {
    ...
    {
      const double side = common::math::CrossProd(point, points_[i], points_[j]);
...
{
  //   CHECK_GE(points_.size(), 3);
  if (polygon.max_x() < min_x() || polygon.min_x() > max_x() || polygon.max_y() < min_y() || polygon.min_y() > max_y())
  {
    return false;
  }
...
void Polygon2d::BuildFromPoints()
{
  ...
  {
    area_ += common::math::CrossProd(points_[0], points_[i - 1], points_[i]);
  }
  if (area_ < 0)
  {
    area_ = -area_;
  ...
  for (int i = 0; i < num_points_; ++i)
  {
    if (common::math::CrossProd(points_[Prev(i)], points_[i], points_[Next(i)]) <= -kMathEpsilon)
    {
      is_convex_ = false;
      break;
    }
...
bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d>& points, Polygon2d* const polygon)
{
  ...
  for (int i = 0; i < n + n; ++i)
  {
    ...
    while (count > last_count &&
           common::math::CrossProd(points[results[count - 2]], points[results[count - 1]], pt) <= kMathEpsilon)
    {
      results.pop_back();
      --count;
    }
...
bool Polygon2d::ClipConvexHull(const LineSegment2d& line_segment, std::vector<Vec2d>* const points)
{
  ...
  {
    prod[i] = common::math::CrossProd(line_segment.start(), line_segment.end(), (*points)[i]);
...
Box2d Polygon2d::BoundingBoxWithHeading(const double heading) const
{
  ...
  const double y1 = py1.CrossProdVec(direction_vec);
  const double y2 = py2.CrossProdVec(direction_vec);
...
Polygon2d Polygon2d::ExpandByDistance(const double distance) const
{
  ...
  for (int i = 0; i < num_points_; ++i)
  {
    ...
    const double diff = common::math::WrapAngle(end_angle - start_angle);
