...
bool LineSegment2d::IsPointIn(const Vec2d& point) const
{
  ...
  const double prod = common::math::CrossProd(point, start_, end_);
...
{
  return unit_direction_.CrossProdVec(point - start_);
}
...
bool LineSegment2d::GetIntersect(const LineSegment2d& other_segment, Vec2d* const point) const
{
  ...
  const double cc1 = common::math::CrossProd(start_, end_, other_segment.start());
  const double cc2 = common::math::CrossProd(start_, end_, other_segment.end());
  if (cc1 * cc2 >= -kMathEpsilon)
  {
    return false;
  }
  const double cc3 = common::math::CrossProd(other_segment.start(), other_segment.end(), start_);
  const double cc4 = common::math::CrossProd(other_segment.start(), other_segment.end(), end_);
